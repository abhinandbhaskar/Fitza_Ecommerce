...
# class GetBillAPIView(APIView):
#     """
#     API View to generate a PDF invoice matching the provided template.
#     """

#     def get(self, request, order_id):
#         try:
#             # Fetch order and bill data
#             order = ShopOrder.objects.get(id=order_id)
#             bill = Bill.objects.get(order=order)
#             user = order.user  # Assuming user details are linked to the order
...
#             # Seller Details (Left)
#             pdf.drawString(50, height - 80, "Your Business Name")
#             pdf.drawString(50, height - 95, "Your Address")
#             pdf.drawString(50, height - 110, "Your Phone Number")
#             pdf.drawString(50, height - 125, "Your Email")
...
#             # pdf.drawRightString(width - 50, height - 110, f"Payment Due: {bill.due_date.strftime('%m/%d/%Y')}")
...
#             pdf.setFont("Helvetica", 10)
#             pdf.drawString(50, height - 175, f"{user.full_name or 'Buyer Name'}")
#             pdf.drawString(50, height - 190, f"{user.shipping_address or 'Buyer Address'}")
#             pdf.drawString(50, height - 205, f"{user.phone or 'Buyer Phone'}")
#             pdf.drawString(50, height - 220, f"{user.email or 'Buyer Email'}")

#             # --- Line Items Table ---
#             pdf.line(50, height - 240, width - 50, height - 240)
#             pdf.setFont("Helvetica-Bold", 10)
#             pdf.drawString(50, height - 255, "Item")
#             pdf.drawString(250, height - 255, "Quantity")
#             pdf.drawString(350, height - 255, "Price per unit")
#             pdf.drawString(450, height - 255, "Amount")
#             pdf.line(50, height - 260, width - 50, height - 260)

#             # Sample items (replace with actual order items)
#             y_position = height - 280
#             for item in order.items.all():  # Assuming order has related items
#                 pdf.setFont("Helvetica", 10)
#                 pdf.drawString(50, y_position, f"{item.product.name}")
#                 pdf.drawString(250, y_position, f"{item.quantity}")
#                 pdf.drawString(350, y_position, f"{bill.currency} {item.price:.2f}")
#                 pdf.drawString(450, y_position, f"{bill.currency} {item.price * item.quantity:.2f}")
#                 y_position -= 20
...
#             pdf.drawString(450, y_position - 40, f"{bill.currency} {bill.subtotal:.2f}")
#             pdf.drawString(350, y_position - 60, f"Tax ({bill.tax_rate}%):")
#             pdf.drawString(450, y_position - 60, f"{bill.currency} {bill.tax:.2f}")
#             pdf.drawString(350, y_position - 80, "Fees:")
#             pdf.drawString(450, y_position - 80, f"{bill.currency} {bill.fees:.2f}")
#             pdf.drawString(350, y_position - 100, "Discounts:")
#             pdf.drawString(450, y_position - 100, f"-{bill.currency} {bill.discount:.2f}")
#             pdf.line(350, y_position - 110, width - 50, y_position - 110)
#             pdf.setFont("Helvetica-Bold", 12)
#             pdf.drawString(350, y_position - 130, "TOTAL:")
#             pdf.drawString(450, y_position - 130, f"{bill.currency} {bill.total_amount:.2f}")

#             # --- Footer: Terms & Notes ---
#             pdf.setFont("Helvetica", 8)
#             pdf.drawString(50, y_position - 160, "Terms and Conditions:")
#             pdf.drawString(50, y_position - 175, bill.terms or "Payment due within 15 days. Late fees may apply.")
...
#             # Seller Details (Left)
#             pdf.drawString(50, height - 80, "Your Business Name")  # Replace with actual business name
#             pdf.drawString(50, height - 95, "Your Business Address")
#             pdf.drawString(50, height - 110, "Your Contact Number")
#             pdf.drawString(50, height - 125, "Your Email")

#             # Invoice Metadata (Right)
#             pdf.drawRightString(width - 50, height - 80, f"Invoice Number: {bill.invoice_number}")
#             pdf.drawRightString(width - 50, height - 95, f"Invoice Date: {bill.bill_date.strftime('%m/%d/%Y')}")
#             pdf.drawRightString(width - 50, height - 110, f"Order Date: {order.order_date.strftime('%m/%d/%Y')}")
...
#             # Billing address from Bill model
#             if bill.billing_address:
#                 pdf.drawString(50, height - 175, f"{bill.billing_address.full_name}")
#                 pdf.drawString(50, height - 190, f"{bill.billing_address.street_address}")
#                 pdf.drawString(50, height - 205, f"{bill.billing_address.city}, {bill.billing_address.state} {bill.billing_address.postal_code}")
#                 pdf.drawString(50, height - 220, f"{bill.billing_address.phone}")
#                 pdf.drawString(50, height - 235, f"{order.user.email}")
#             else:
...
#                 pdf.setFont("Helvetica", 10)
#                 pdf.drawString(50, height - 275, f"{shipping.shipping_address.full_name}")
#                 pdf.drawString(50, height - 290, f"{shipping.shipping_address.street_address}")
...
#             # Actual order items
#             y_position -= 50
#             for item in order.items.all():  # Assuming you have an items relationship
#                 pdf.setFont("Helvetica", 10)
#                 pdf.drawString(50, y_position, f"{item.product.name}")
#                 pdf.drawString(250, y_position, f"{item.quantity}")
#                 pdf.drawString(350, y_position, f"{bill.currency} {item.price:.2f}")
...
#             # Subtotal
#             pdf.drawString(350, y_position - 40, "Subtotal:")
#             pdf.drawString(450, y_position - 40, f"{bill.currency} {order.order_total:.2f}")
            
#             # Shipping
#             if shipping and shipping.shipping_cost:
#                 pdf.drawString(350, y_position - 60, "Shipping:")
#                 shipping_cost = 0.00 if order.free_shipping_applied else shipping.shipping_cost
#                 pdf.drawString(450, y_position - 60, f"{bill.currency} {shipping_cost:.2f}")
#                 if order.free_shipping_applied:
#                     pdf.drawString(350, y_position - 80, "(Free Shipping Applied)")
            
#             # Tax
#             pdf.drawString(350, y_position - 100, f"Tax:")
#             pdf.drawString(450, y_position - 100, f"{bill.currency} {order.tax_amount:.2f}")
            
#             # Discount
#             pdf.drawString(350, y_position - 120, "Discount:")
#             pdf.drawString(450, y_position - 120, f"-{bill.currency} {order.discount_amount:.2f}")
#             if order.applied_coupon:
#                 pdf.drawString(350, y_position - 140, f"Coupon: {order.applied_coupon.code}")
            
#             # Grand Total
#             pdf.line(350, y_position - 150, width - 50, y_position - 150)
#             pdf.setFont("Helvetica-Bold", 12)
#             pdf.drawString(350, y_position - 170, "TOTAL:")
#             pdf.drawString(450, y_position - 170, f"{bill.currency} {order.final_total:.2f}")
...
#             # --- Footer: Terms & Notes ---
#             pdf.setFont("Helvetica", 8)
#             pdf.drawString(50, y_position - 280, "Terms and Conditions:")
#             terms = bill.notes or "Payment due upon receipt. Late payments may incur fees."
#             pdf.drawString(50, y_position - 295, terms)
