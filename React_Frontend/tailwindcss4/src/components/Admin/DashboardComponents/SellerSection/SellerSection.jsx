    ...
    const approveSeller = async (seller_id) => {
        try {
            const response = await axios.post(
                `https://127.0.0.1:8000/api/admin/approve_seller/${seller_id}/`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                                    ...
                                    {filteredSellers && filteredSellers.length > 0 ? (
                                        filteredSellers.map((sellers, key) => (
                                            <tr key={sellers.id} className="hover:bg-gray-100 transition duration-200">
                                                ...
                                                <td className="px-6 py-4 text-sm text-gray-700 border-b border-gray-300">
                                                    <button
                                                        onClick={() => ViewSellerDetails(sellers.id)}
                                                ...
                                                <td
                                                    className={`px-6 py-4 text-sm ${
                                                        sellers.user.is_active ? "text-green-600" : "text-red-600"
                                    ...
                                    {approvals && approvals.length > 0 ? (
                                        approvals.map((sellers, key) => (
                                            <tr key={sellers.id} className="hover:bg-gray-100 transition duration-200">
                                                ...
                                                <td
                                                    className={`px-6 py-4 text-sm ${
                                                        safe(sellers, "user.is_active") ? "text-green-600" : "text-red-600"
                                                    } border-b border-gray-300`}
                                                >
                                ...
                                <div className="flex items-center space-x-6">
                                    ...
                                    <div>
                                        <p className="text-lg font-semibold">
                                            Full Name: {safe(seller, "user.first_name") || "Not available"}
                                        </p>
                            ...
                            <div className="bg-white rounded-2xl shadow-md p-6 mb-8">
                                ...
                                <div className="space-y-2">
                                    <p>
                                        ...
                                        {safe(seller, "shop_name") || "Not available"}
                                    </p>
                                    <p>
                                        <strong>Shop Address:</strong> {safe(seller, "shop_address") || "Not available"}
                                    </p>
                                    <p>
                                        <strong>Contact Number:</strong> {safe(seller, "contact_number") || "Not available"}
                                    </p>
                                    <p>
                                        <strong>Shop Email:</strong> {safe(seller, "email") || "Not available"}
                                    </p>
                                    <p>
                                        <strong>Tax ID:</strong>
                                        {safe(seller, "tax_id") || "Not available"}
                                    </p>
                                    <p>
                                        <strong>Business Registration Number:</strong>
                                        {safe(seller, "business_registration_number") || "Not available"}
                                    </p>
                                    <p>
                                        <strong>Description:</strong> {safe(seller, "description") || "Not available"}
                                    </p>
                                </div>
                            </div>
                                ...
                                {seller.bank_details ? (
                                    <div className="space-y-2">
                                        <p>
                                            ...
                                            {safe(seller, "bank_details.account_holder_name")}
                                        </p>
                                        <p>
                                            <strong>Bank Name:</strong> {safe(seller, "bank_details.bank_name")}
                                        </p>
                                        <p>
                                            <strong>Account Number:</strong> {safe(seller, "bank_details.account_number")}
                                        </p>
                                        <p>
                                            <strong>IFSC Code:</strong> {safe(seller, "bank_details.ifsc_code")}
                                        </p>
                                        <p>
                                            <strong>Branch Address:</strong> {safe(seller, "bank_details.branch_address")}
                                        </p>
                                    </div>
                                ) : (
