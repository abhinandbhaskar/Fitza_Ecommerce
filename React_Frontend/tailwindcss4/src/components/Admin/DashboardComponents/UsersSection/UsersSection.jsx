    ...
    const RemoveUser = async (user_id) => {
        try {
            ...
            const response = await axios.post(
                `https://127.0.0.1:8000/api/admin/remove_users/${user_id}/`,
                {},
                {
                    headers: {
                                ...
                                {filteredUsers.length > 0 ? (
                                    filteredUsers.map((users, key) => (
                                        ...
                                        >
                                            ...
                                            <td
                                                className={`px-6 py-4 text-sm font-semibold border-b border-gray-300 ${
                                                    safe(users, "is_active", false) ? "text-green-600" : "text-red-600"
                                                }`}
                                            >
                                                {safe(users, "is_active", false) ? "Active" : "Inactive"}
                                            </td>
                                            <td className="px-6 py-4 text-sm text-gray-700 border-b border-gray-300">
                                                {safe(users, "date_joined")
                                                    ? new Date(safe(users, "date_joined")).toLocaleString()
                                                    : "N/A"}
                                            </td>
                                            <td className="px-6 py-4 text-sm border-b border-gray-300">
                                                <button
                                                    onClick={() => handleRemove(safe(users, "id", null))}
                                                    className="bg-red-500 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 shadow hover:bg-red-600 hover:shadow-md transition-transform transform duration-200 hover:scale-105"
                                                >
