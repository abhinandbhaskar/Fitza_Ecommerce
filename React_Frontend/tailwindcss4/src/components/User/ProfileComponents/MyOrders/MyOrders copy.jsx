          ...
          <tbody>
                {filteredOrders.map((order,key) => (
                  <tr key={key} className="border-t hover:bg-gray-100">
                    ...
                    <td className="px-6 py-4 text-sm font-semibold">
                      <span className="text-green-600">{safe(order,'order_status.status')}</span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-800">
                      {safe(order, "order_date") ? new Date(safe(order, "order_date")).toLocaleString() : "N/A"}

                    </td>
                    <td className="px-6 py-4 text-sm text-gray-800">Rs. {safe(order,'final_total')}</td>
...
{/* Order Summary */}
          <div className="bg-white rounded-lg shadow-md p-4 my-4">
            ...
            <div className="mt-2 text-gray-600">
              <p><span className="font-semibold">Order ID:</span> {safe(details,'id')}</p>
...
>
  ...
  <div className="flex-1 text-sm md:text-base">
    <h3 className="font-semibold truncate">{safe(product,'product_item.product.product_name') || "Not Available.."}</h3>
    <p className="text-gray-600 truncate">{safe(product,'product_item.product.product_description') || "Not Available.."}</p>
  
    <p>
      <span className="font-semibold">Quantity:</span> {safe(product,'quantity') || "Not Available.."}
    </p>
    <p>
      <span className="font-semibold">Price:</span> Rs.{safe(product,'price') || "Not Available.."}
    </p>
  </div>
...
<h3 className="text-md font-bold text-gray-700">{safe(address,'user.first_name')+safe(address,'user.last_name')}</h3>
<p className="mt-2 text-gray-600">{safe(address,'address_line1')},{safe(address,'address_line2')},pin {safe(address,'postal_code')},{safe(address,'city')},{safe(address,'state')},{safe(address,'country')}</p>
...
<p>Delivery Updates : {safe(details,'shipping_address.status')}</p>
</div>
...
<div className="mt-2 text-gray-600">
  ...
  <p className="font-semibold mt-2">
    ...
    <p className="font-semibold mt-2">
    <span className="text-gray-800"> Shipping Fee:</span> {safe(details,'shipping_address.shipping_cost')==0?"Free":safe(details,'shipping_address.shipping_cost')}
  </p>
    <p className="font-semibold mt-2">
    <span className="text-gray-800">Platform fee:</span> {safe(details,'payment_method.platform_fee')}
  </p>
  <p className="font-semibold mt-2">
<span className="text-gray-800">Coupon:</span> {safe(details,'applied_coupon.discount_type') === "fixed" ? `-${safe(details,'applied_coupon.discount_value')}` : `${safe(details,'applied_coupon.discount_value')}%`}
  </p>
    <p className="font-semibold mt-2">
