...
const MainSection = ({setCurrentView}) => {
  ...
  // Process sales data for daily chart
  const processDailyData = () => {
    if (!salesData.length) return { labels: [], datasets: [] };
    
    // Group by day of week (simplified - in a real app you'd want to group by actual dates)
    ...
    // Rotate array to start with Monday (index 1)
  ...
  // Process sales data for weekly trend chart
  const processWeeklyTrendData = () => {
    if (!salesData.length) return { labels: [], datasets: [] };
    
    // Group by week (simplified - in a real app you'd want proper week grouping)
    const weeklySales = [0, 0, 0, 0, 0, 0]; // Last 6 weeks
    
    // This is a simplified example - you'd need to properly group by week
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-xs p-4 h-full">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-1.5 mt-2">
                        <div 
                          ...
                          style={{ width: `${topProducts.length > 0 ? (product.sales / topProducts[0].sales) * 100 : 0}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
            ...
            <button onClick={()=>setCurrentView("products")} className="w-full mt-4 px-4 py-2 text-sm font-medium text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded-md transition-colors">
              View All Products →
            </button>
          </div>
        </div>
